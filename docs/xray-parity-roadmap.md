...
| ~~**P4**~~ | ~~Kernel TLS splice (`SO_KTLS`)~~                             | Linux `setsockopt TCP_ULP "tls"` + `SOL_TLS` key install after rustls handshake             | Linux kernel 4.17+; graceful fallback on old kernels             | **Implemented** — `tls_accept()` offloads AES-128/256-GCM to kernel; enables `splice(2)` relay path |
