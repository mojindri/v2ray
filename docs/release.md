...
- TUN transparent proxy on Linux, covered by privileged CI tests; Linux outbound sockets use `SO_MARK`; macOS utun runtime installs split default routes plus a PF anchor for TCP/DNS redirection and uses `tun.outboundInterface`/`tun.outbound_interface` for protected proxy egress; Windows Wintun device creation compiles through the native backend and can use `tun.wintunFile`/`tun.wintun_file` to point at a bundled `wintun.dll`; shared packet/NAT/session APIs and the runtime packet loop compile cross-platform; full-device runtime support is reported through an explicit platform support contract
...
- Windows full-device TUN/Wintun runtime — device backend, explicit `wintun.dll` path wiring, and split-route setup are present, but runtime support still requires native TCP redirection
