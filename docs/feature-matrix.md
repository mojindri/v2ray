...
| TUN transparent proxy                  | **Supported (Linux)** | Linux TUN runtime via `transport/tun/` when `config.tun` is set; route setup/cleanup and UDP NAT covered by privileged `tun_priv.rs`; macOS utun and Windows Wintun device creation compile through the native `tun` crate backend; Windows can use `tun.wintunFile`/`tun.wintun_file` to point at a bundled `wintun.dll`; shared packet/NAT/session APIs compile cross-platform; `current_tun_support()`/`ensure_tun_runtime_supported()` make full-device runtime support explicit; macOS/Windows still need native routing and TCP redirection |
