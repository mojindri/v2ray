...
| TUN transparent proxy                  | **Supported (Linux)** | Linux TUN runtime via `transport/tun/` when `config.tun` is set; route setup/cleanup and UDP NAT covered by privileged `tun_priv.rs`; Linux outbound sockets use `SO_MARK`; macOS can configure `tun.outboundInterface`/`tun.outbound_interface` for protected outbound sockets; macOS utun and Windows Wintun device creation compile through the native `tun` crate backend; Windows can use `tun.wintunFile`/`tun.wintun_file` to point at a bundled `wintun.dll`; shared packet/NAT/session APIs and the runtime packet loop compile cross-platform; `current_tun_support()`/`ensure_tun_runtime_supported()` make full-device runtime support explicit; macOS/Windows still need native routing and TCP redirection |
...
| macOS (Apple Silicon / Intel) | **Supported**   | CI runs `macos-latest` tests; core proxy runtime is supported; utun device creation compiles, but full-device TUN runtime is still gated |
| Windows                       | **Partial**     | CI runs Windows workspace tests; core proxy runtime is intended to be portable; Wintun device creation compiles and accepts an explicit `wintun.dll` path, but full-device TUN runtime is still gated |
