...
| TUN transparent proxy                  | **Supported (Linux)** | Linux TUN runtime via `transport/tun/` when `config.tun` is set; route setup/cleanup and UDP NAT covered by privileged `tun_priv.rs`; macOS utun and Windows Wintun device creation compile through the native `tun` crate backend; shared packet/NAT/session APIs compile cross-platform; `current_tun_support()`/`ensure_tun_runtime_supported()` make full-device runtime support explicit; macOS/Windows still need native routing, TCP redirection, and Windows Wintun DLL packaging |
...
| Windows                       | **Partial**     | CI runs `cargo check --workspace --all-targets --locked`; core proxy runtime is intended to be portable; Wintun device creation compiles, but full-device TUN runtime is still gated |
