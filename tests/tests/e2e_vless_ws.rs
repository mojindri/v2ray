//! Runnable VLESS-over-WebSocket example test: SOCKS5 client -> VLESS -> WebSocket -> Freedom.
//!
//! This test mirrors `examples/vless-ws-local`. It proves that the
    ...
    parse_config(format!(
        r#"{{
            ...
            "inbounds": [{{
                ...
                "port": {vless_port},
                "settings": {{
                    "clients": [{{ "id": "{TEST_UUID}", "email": "vless-ws@example.test" }}]
                }},
                "streamSettings": {{
                    "network": "ws",
                    "security": "none",
...
#[tokio::test]
async fn vless_over_ws_to_freedom_transfers_data() {
