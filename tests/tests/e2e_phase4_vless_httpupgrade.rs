//! Phase 4: SOCKS5 -> VLESS over HTTPUpgrade -> Freedom echo.
        ...
        r#"{{
            ...
            "inbounds": [{{
                ...
                "streamSettings": {{
                    "network": "httpupgrade",
                    "security": "none",
                    "httpupgradeSettings": {{ "path": "/phase4-hu" }}
                }}
            }}],
...
fn client_config(socks_port: u16, vless_port: u16) -> Arc<blackwire_config::schema::Config> {
    parse_config(format!(
        r#"{{
            ...
            "routing": {{ "rules": [{{ "outboundTag": "vless-hu-out" }}] }}
        }}"#
    ))
}

#[tokio::test]
async fn phase4_vless_over_httpupgrade_to_freedom_transfers_data() {
    ...
    let payload = b"HELLO PHASE4 VLESS HTTPUPGRADE";
