//! Runnable Phase 5 example test: HTTP CONNECT -> VMess -> gRPC -> Freedom.
//!
//! This mirrors `examples/phase5-http-vmess-grpc-local` and proves that real
//! bytes can cross the full Phase 5 chain.
...
const GRPC_SERVICE: &str = "phase5.Gun";
    ...
    parse_config(format!(
        r#"{{
            ...
            "inbounds": [{{
                ...
                "port": {vmess_port},
                "settings": {{
                    "clients": [{{ "id": "{TEST_UUID}", "email": "phase5@example.test" }}]
                }},
                "streamSettings": {{
                    "network": "grpc",
                    "security": "none",
...
#[tokio::test]
async fn phase5_http_connect_vmess_grpc_transfers_data() {
