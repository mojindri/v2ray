//! Runnable Phase 8 example test: SOCKS5 -> VLESS -> mKCP -> Freedom.
//!
    ...
    parse_config(format!(
        r#"{{
            ...
            "inbounds": [{{
                ...
                "port": {vless_port},
                "settings": {{
                    "clients": [{{ "id": "{TEST_UUID}", "email": "phase8@example.test" }}]
                }},
                "streamSettings": {{
                    "network": "kcp",
                    "security": "none",
...
#[tokio::test]
async fn phase8_vless_over_mkcp_transfers_data() {
...
#[tokio::test]
async fn phase8_vless_over_mkcp_accepts_concurrent_sessions() {
    let _ = tracing_subscriber::fmt().with_env_filter("warn").try_init();
