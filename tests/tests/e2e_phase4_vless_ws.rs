//! Runnable Phase 4 example test: SOCKS5 client -> VLESS -> WebSocket -> Freedom.
//!
//! This test mirrors `examples/phase4-vless-ws-local`. It proves that the
//! example is not only valid JSON: real bytes enter the SOCKS5 listener, cross
//! VLESS over WebSocket, hit a TCP echo server through Freedom, and come back.
...
async fn socks5_connect(socks_port: u16, dest_host: &str, dest_port: u16) -> TcpStream {
    ...
    // Step 1: no-auth SOCKS5 handshake.
    stream.write_all(&[5, 1, 0]).await.unwrap();
    let mut resp = [0u8; 2];
    stream.read_exact(&mut resp).await.unwrap();
    assert_eq!(resp, [5, 0]);

    // Step 2: ask the proxy to connect to the final destination.
    ...
    parse_config(format!(
        r#"{{
            ...
            "inbounds": [{{
                ...
                "port": {vless_port},
                "settings": {{
                    "clients": [{{ "id": "{TEST_UUID}", "email": "phase4@example.test" }}]
                }},
                "streamSettings": {{
                    "network": "ws",
                    "security": "none",
                    "wsSettings": {{ "path": "/phase4" }}
                }}
            }}],
...
#[tokio::test]
async fn phase4_vless_over_ws_to_freedom_transfers_data() {
    ...
    // Give both listeners a moment to bind before the test client connects.
    tokio::time::sleep(tokio::time::Duration::from_millis(50)).await;

    let mut stream = socks5_connect(socks_port, "127.0.0.1", echo_port).await;
    let payload = b"HELLO PHASE4 VLESS WS";
