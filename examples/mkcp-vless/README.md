...
```sh
cargo run -q -p blackwire -- test -c examples/mkcp-vless/client.json
cargo run -q -p blackwire -- test -c examples/mkcp-vless/server.json
cargo test -p integration-tests vless_over_mkcp
```
