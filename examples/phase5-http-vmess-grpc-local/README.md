# Phase 5 HTTP CONNECT to VMess over gRPC Local Example

This example shows a local Phase 5 data path:
...
service name `phase5.Gun`, authenticates the shared UUID, then uses `freedom` to
...
```sh
cargo test -p integration-tests phase5_http_connect_vmess_grpc_transfers_data
```
