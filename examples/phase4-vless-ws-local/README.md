# Phase 4 VLESS over WebSocket Local Example

This example shows a local Phase 4 data path:
...
path `/phase4`, authenticates the shared UUID, then uses `freedom` to connect to
...
```sh
cargo test -p integration-tests phase4_vless_over_ws_to_freedom_transfers_data
```
